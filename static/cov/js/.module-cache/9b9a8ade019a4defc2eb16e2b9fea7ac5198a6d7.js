...
function User(name) {
  ...
  this.history = [];
  // TODO hard code the first one
  this.currentPrompt = {"type": PROMPT_TEXT, "text": "First Question", "workflowStateID": "2"};
  // this.oldCart = null;
  // this.newCart = null;
  // this.results = null;
...
User.prototype = {
  ...
  getWorkflowStateID: function() {
    if (!this.currentPrompt) {
      return this.currentPrompt.workflowStateID;
    }
  },

  updateUser: function(j) {
    self.history = j["messages"];
    self.currentPrompt = j["prompt"];    
  },
  ...
  loadHistory: function() {
    ...
    var promise = new Promise(function(resolve, reject) {
      ...
      historyReq.onload = function() {
        updateUser(JSON.parse(historyReq.responseText));
        resolve();
        // resolve(self);
      };
  ...
  //Reload history
  submitTextResponse: function(workflowStateID, text, renderCallback) {
    var self = this;

    var formData = new FormData();

    formData.append("user", this.username);
    formData.append("response", "abc");
    formData.append("workflowStateID", workflowStateID);

    var responsePromise = new Promise(function(resolve, reject) {
      var xhr = new XMLHttpRequest();
      xhr.onload = function() {
        updateUser(JSON.parse(xhr.responseText));
        resolve();
      };
      xhr.error = function() {
        reject();
