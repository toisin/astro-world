...
User.prototype = {
  ...
  loadAllUserData: function(username, callback) {
    debugger;
    challengePromise = this.loadUserChallengeData(username);

    challengePromise.then(callback, function(error) {
                                      console.error("Fine!", error);
                                    });
  },
  ...
  loadUserChallengeData: function(username, callback) {
    ...
    var promise = new Promise(function(resolve, reject) {
      ...
      challengeReq.open('GET', '/userchallenge/' + this.username + '/findallchallenges');
      challengeReq.send(null);

    });

    return promise.then(function(username) {
      return this.loadUserCartData(username);
    });
  },
