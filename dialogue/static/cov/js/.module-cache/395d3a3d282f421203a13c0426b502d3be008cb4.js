...
User.prototype = {
  ...
  loadAllUserData: function(renderCallback) {
    ...
    var challengePromise = user.loadUserChallengeData(user);

    var cartPromise = challengePromise.then(function(){user.loadUserResultData();});
  ...
  loadUserChallengeData: function(user) {
    var promise = new Promise(function(resolve, reject) {
      ...
      challengeReq.open('GET', '/userchallenge/' + user.username + '/findallchallenges');
  ...
  // passing in self because otherwise, the scope can be screwed up if
  //     this is called from Promise
  loadUserResultData: function(user) {
    var promise = new Promise(function(resolve, reject) {
      var resultsReq = new XMLHttpRequest();
      resultsReq.onload = function() {
        debugger;
        user.results = JSON.parse(resultsReq.responseText);
        resolve(user);
      };
      resultsReq.onerror = function() {
        reject(Error("It broke"));
      };
      resultsReq.open('GET', '/usercart/' + user.username + '/findallcarts');
  ...
  addResult: function(result, renderCallback) {
    ...
    var addCartPromise = new Promise(function(resolve, reject) {
      ...
      xhr.open('POST', '/usercart/' + user.username + '/addcartdata');
      xhr.setRequestHeader('Content-Type', 'application/json');
      xhr.send(JSON.stringify(result));
    });

    var loadUserCartPromise = addCartPromise.then(user.loadUserResultData);
