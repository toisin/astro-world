...
var PromptTypes = {PROMPT_ROBOT: 'Researcher', PROMPT_HUMAN : 'Student'};
...
var Title = React.createClass({displayName: 'Title',
  render: function() {
    ...
    React.DOM.tr(null, React.DOM.td(null, "Hello ", this.props.user.getUsername())),
...
var Prompt = React.createClass({displayName: 'Prompt',
  render: function() {
    if (this.props.prompt.type == PROMPT_ROBOT) {
      return React.DOM.tr(null, React.DOM.td(null, PromptTypes[PROMPT_ROBOT]),React.DOM.td(null, this.props.prompt.text));
    } else {
      return React.DOM.tr(null, React.DOM.td(null, PromptTypes[PROMPT_HUMAN]),React.DOM.td(null, this.props.prompt.text));
    }
  }
});
...
var App = React.createClass({displayName: 'App',
  ...
  render: function() {
    ...
    var prompts = user.getPrompts().map(
        function(prompt) {
    ...
    switch (this.state.mode) {
      case 0: //Show User Name
        return React.DOM.table( {className:"app"}, React.DOM.tbody(null, 
          React.DOM.tr(null, 
            React.DOM.td(null, Title( {user:user}))
          ),
          prompts
