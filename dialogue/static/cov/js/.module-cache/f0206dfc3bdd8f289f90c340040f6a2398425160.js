...
var Title = React.createClass({displayName: 'Title',
  render: function() {
    ...
    return  React.DOM.div( {class:"researcher"}, 
              React.DOM.div( {class:"name"}, "Researcher:"),
              React.DOM.div( {class:"message"}, 
...
var Message = React.createClass({displayName: 'Message',
  render: function() {
    ...
    if (message.Mtype == MSG_ROBOT) {
      return  React.DOM.div( {class:"researcher"}, 
                React.DOM.div( {class:"name"}, DisplayText[MSG_ROBOT],":"),
                React.DOM.div( {class:"message"}, message.Text)
              );
    } else if (message.Mtype == MSG_HUMAN) {
      return  React.DOM.div( {class:"user"}, 
                React.DOM.div( {class:"name"}, human,":"),
                React.DOM.div( {class:"message"}, message.Text)
              );
    }
    console.error("Unknown sender!", error);
    return  React.DOM.div( {class:"researcher"}, 
              React.DOM.div( {class:"message"}, this.props.message.Text)
            );
  }
});
  ...
  render: function() {
    ...
    if (type == PROMPT_TEXT) {
      return  React.DOM.div(null, 
                React.DOM.div( {class:"researcher"}, 
                  React.DOM.div( {class:"name"}, DisplayText[MSG_ROBOT],":"),
                  React.DOM.div( {class:"message"}, prompt.Text)
                ),
                React.DOM.div( {class:"user"}, 
                  React.DOM.div( {class:"name"}, human,":"),
                  React.DOM.div( {class:"message"}, 
    ...
    if (type == PROMPT_NO_RESPONSE) {
      return  React.DOM.div(null, DisplayText[MSG_ROBOT], " : ", prompt.Text);
    }
...
var App = React.createClass({displayName: 'App',
  ...
  render: function() {
    ...
    if ((!prompt) || (Object.keys(prompt).length == 0)) {
        return React.DOM.div( {class:"chat"}, 
          Title( {user:user}),
          messages
        );
    } else {
