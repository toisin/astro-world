...
*/
  ...
  render: function() {
    ...
    return React.DOM.form( {onSubmit:this.handleSubmit, onChange:this.handleChange,
            className:"request"}, 
      React.DOM.table(null, React.DOM.tbody(null, "What did you find out?",React.DOM.textarea( {id:"findout"}))),
...
/*      case 'all':
    ...
    var headers = variableModels.iVariables.map(function(iv) {
      return React.DOM.th(null, VariableImage( {name:iv.name}),iv.label);
    });
  ...
  render: function() {
    ...
    switch (this.state.mode) {
      ...
      case 4: //Show Notebook
        return React.DOM.div( {className:"app single-column"}, 
          UserResultData( {variableModels:this.props.variableModels, user:this.props.user, mode:'comparison'}),
          React.DOM.button( {onClick:this.continueFrom}, "Go to Next Case")
        );
